<template>
  <div f-c-c>
    <n-card title='我的额度' bordered mx-10 my-10 rounded-10>
      <div flex relative style='bottom: 3rem'>
        <n-input-number v-model:value='inputValue' clearable='true' placeholder='申请增加今日分割次数' pr-7/>
        <n-button strong='true' @click='addQuota' type='primary'>申请</n-button>
      </div>
...
const { fetchData, quotaApplyInsert } = api
const store = useStore()
const username = localStorage.getItem('username')
// 创建一个数组，表示表格的列定义 createColumns
const inputValue = ref('')
const addQuota = () => {
  if (inputValue.value <= 0 || inputValue.value == null || inputValue.value.toString().includes('.')) {
  ...
  quotaApplyInsert({
    ...
    console.log(data)
    message.success(`申请成功，你申请了${inputValue.value}次`)
    inputValue.value = ''
  })
}
const createColumns = ({
