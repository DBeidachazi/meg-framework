<template>
  <CommonPage>
    <div h-60 flex items-center bg-white pl-20 pr-20>
      ...
      />
      <n-button type='primary' style='width: 80px; margin-right: 10px;' :loading='btnLoading' @click='speak'>
        <TheIcon v-if='!btnLoading' icon='line-md:emoji-grin' class='mr-5' :size='18' />
        朗读
      </n-button>
    ...
    <MdEditor v-model='post.content' style='height: calc(100vh - 305px)' dark:bg-dark :toolbars='toolbars'
              :on-save='handleSavePdf'>
    </MdEditor>
  </CommonPage>
</template>
...
import { exportPDF } from './exportMedicalCertificate'

const speech = new Speech()

const handleSavePdf = () => {
  // isLoad true: 加载到新页面 false: 下载
  exportPDF("md-editor-v3-preview", "pdf", true, "MedSego", "#ffffff", 3)
}
